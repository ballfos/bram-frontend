...
const pieceSymbols = {
  p:  <img src="/hitomoji/black_pawn.png" alt="Black Pawn" className="piece-image" />, l: <img src="/hitomoji/black_lance.png" alt="Black Pawn" className="piece-image" />,
   n:<img src="/hitomoji/black_knight.png" alt="Black Pawn" className="piece-image" /> , s: <img src="/hitomoji/black_silver.png" alt="Black Pawn" className="piece-image" />, g:  <img src="/hitomoji/black_gold.png" alt="Black Pawn" className="piece-image" />, k: <img src="/hitomoji/black_king.png" alt="Black Pawn" className="piece-image" />, r: <img src="/hitomoji/black_rook.png" alt="Black Pawn" className="piece-image" />, b:  <img src="/hitomoji/black_bishop.png" alt="Black Pawn" className="piece-image" />,
  "+p": <img src="/hitomoji/black_prom_pawn.png" alt="Black Pawn" className="piece-image" />, "+l":  <img src="/hitomoji/black_prom_lance.png" alt="Black Pawn" className="piece-image" />, "+n": <img src="/hitomoji/black_prom_knight.png" alt="Black Pawn" className="piece-image" />, "+s": <img src="/hitomoji/black_prom_silver.png" alt="Black Pawn" className="piece-image" />, "+r": <img src="/hitomoji/black_dragon.png" alt="Black Pawn" className="piece-image" />, "+b": <img src="/hitomoji/black_horse.png" alt="Black Pawn" className="piece-image" />,
  P:  <img src="/hitomoji/black_pawn.png" alt="Black Pawn" className="piece-image" />, L: <img src="/hitomoji/black_lance.png" alt="Black Pawn" className="piece-image" />, 
  N: <img src="/hitomoji/black_knight.png" alt="Black Pawn" className="piece-image" />, S: <img src="/hitomoji/black_silver.png" alt="Black Pawn" className="piece-image" />, G:<img src="/hitomoji/black_gold.png" alt="Black Pawn" className="piece-image" />, K: <img src="/hitomoji/black_king2.png" alt="Black Pawn" className="piece-image" />, R:  <img src="/hitomoji/black_rook.png" alt="Black Pawn" className="piece-image" />, B: <img src="/hitomoji/black_bishop.png" alt="Black Pawn" className="piece-image" />,
  "+P": <img src="/hitomoji/black_prom_pawn.png" alt="Black Pawn" className="piece-image" />, "+L": <img src="/hitomoji/black_prom_lance.png" alt="Black Pawn" className="piece-image" />, "+N": <img src="/hitomoji/black_prom_knight.png" alt="Black Pawn" className="piece-image" />, "+S": <img src="/hitomoji/black_prom_silver.png" alt="Black Pawn" className="piece-image" />, "+R": <img src="/hitomoji/black_dragon.png" alt="Black Pawn" className="piece-image" />, "+B": <img src="/hitomoji/black_horse.png" alt="Black Pawn" className="piece-image" />,
};
...
const sfenToNewBoard = (sfen) => {
  ...
  if (handString && handString !== "-") {
    ...
    for (const piece of playerPieces) {
      const count = parseInt(piece) || 1; // 数字部分がなければ1個
...
const initialSfen = "lnsgkgsnl/1r5b1/ppppppppp/9/9/9/PPPPPPPPP/1B5R1/LNSGKGSNL w - 1";
const sfenString = "lnsgkg1nl/1r5s1/ppp1p2pp/3p5/5PP2/2P6/PP1PP2PP/7R1/LNSGKGSNL b bB2P w - 3";
...
const ShogiBoard = () => {
  ...
  const sendData = async () => {   //SFEN送信
    try {
        const response = await axios.post("http://127.0.0.1:5000/api/data", { currentSfen });
  ...
  const isEnemyPiece = (piece, turn) => {
    return piece && ((turn === "b" && piece === piece.toUpperCase()) || (turn === "w" && piece === piece.toLowerCase()));
  };

  const isOwnPiece = (piece, turn) => {
    return piece && ((turn === "b" && piece === piece.toLowerCase()) || (turn === "w" && piece === piece.toUpperCase()));
  };

  const promotePiece = (piece) => {
    if (["p", "l", "n", "s", "r", "b", "P", "L", "N", "S", "R", "B"].includes(piece)) {
      return `+${piece}`;
    }
    return piece;
  };
    ...
    if (selectedHandPiece !== null) {
      if (!board[row][col]) {
        ...
        setHand((prev) => {
          ...
          updatedHand[turn] = prev[turn].filter((_, index) => index !== selectedHandPiece.index);
    ...
    if (selectedPiece) {
      ...
      if (movable) {
        ...
        if (isEnemy) {
          ...
          setHand((prev) => {
            const updatedHand = { ...prev };
            const originalPiece = piece.startsWith("+") ? piece.slice(1) : piece; // 成駒を元の駒に戻す
            if (turn === "b") {
        ...
        const isPromotable = ["p", "l", "n", "s", "r", "b", "P", "L", "N", "S", "R", "B"].includes(movingPiece);
  ...
  const handleHandClick = (piece, index) => {
    if ((turn === "b" && piece === piece.toLowerCase()) || (turn === "w" && piece === piece.toUpperCase())) {
  ...
  const getMovableCells = (row, col, piece) => {
    ...
    const directions = {
      ...
      s: [[1, 1], [1, -1], [1, 0]],
      S: [[-1, 1], [-1, -1], [-1, 0]],
      g: [[1, 0], [0, 1], [0, -1], [-1, 0], [1, 1], [1, -1]],
      G: [[-1, 0], [0, 1], [0, -1], [1, 0], [-1, 1], [-1, -1]],
      k: [[1, 0], [0, 1], [0, -1], [-1, 0], [1, 1], [1, -1], [-1, 1], [-1, -1]],
      K: [[1, 0], [0, 1], [0, -1], [-1, 0], [1, 1], [1, -1], [-1, 1], [-1, -1]],
      r: [[1, 0], [-1, 0], [0, 1], [0, -1]],
      R: [[1, 0], [-1, 0], [0, 1], [0, -1]],
      b: [[1, 1], [-1, -1], [1, -1], [-1, 1]],
      B: [[1, 1], [-1, -1], [1, -1], [-1, 1]],
      "+p": [[1, 0], [0, 1], [0, -1], [-1, 0], [1, 1], [1, -1]],
      "+P": [[-1, 0], [0, 1], [0, -1], [1, 0], [-1, 1], [-1, -1]],
      "+l": [[1, 0], [0, 1], [0, -1], [-1, 0], [1, 1], [1, -1]],
      "+L": [[-1, 0], [0, 1], [0, -1], [1, 0], [-1, 1], [-1, -1]],
      "+n": [[1, 0], [0, 1], [0, -1], [-1, 0], [1, 1], [1, -1]],
      "+N": [[-1, 0], [0, 1], [0, -1], [1, 0], [-1, 1], [-1, -1]],
    ...
    (directions[piece] || []).forEach(([dx, dy]) => {
      ...
      while (x >= 0 && x < 9 && y >= 0 && y < 9) {
        ...
        else if(["+r", "+R", "+b", "+B"].includes(piece)){ //竜、馬用処理
           if(num < 5){
            x += dx;
            y += dy;
           }
